...
'vcard': `BEGIN:VCARD\nVERSION:3.0\nN:XL;ttname,;;;\nFN:ttname\nitem1.TEL;waid=254799073744:+254769279076\nitem1.X-ABLabel:Ponsel\nEND:VCARD`,
sendEphemeral: true
}}
}
...
if (autobio) {
  malvin.updateProfileStatus(` ⎋ESCANOR  𝙱𝙾𝚃 is Online✅ Runtime ${runtime(process.uptime())}`)
    ...
    return malvin.sendMessage(m.chat, {
      ...
      contextInfo: {
        ...
        externalAdReply: {
          ...
          thumbnailUrl: "https://i.ibb.co/rRg9wTZV/malvin-xd.jpg" ,               
...
//////////////runtime///////////////////////
    case 'uptime':
    case 'runtime': {
      m.reply(`⏰️ ${runtime(process.uptime())}`)
    }
...
case 'play': {
  ...
  try {
    ...
    const outputFileName = `${search.all[0].title.replace(/[^a-zA-Z0-9 ]/g, "")}.mp3`;
    const outputPath = path.join(tempDir, `${timestamp}.mp3`);

    for (const api of apis) {
      try {
        // Use axios to get JSON response
        ...
        if (data.status === 200 || data.success) {
          ...
          if (response.status !== 200) {
            m.reply("Sorry, API endpoint didn't respond correctly. Try again later.");
            continue;
          }

          // Convert stream to mp3 and save to outputPath
          ...
          // Send audio file as document (recommended for mp3)
